...
class HaloManager:
    ...
    def update_ticket(self, zendesk_request: dict = None) -> dict:
        ...
        :raises:
            HelpDeskTicketNotFoundException: If no ticket is found.
        """
        logger.info(f"HaloManager.update_ticket zendesk_request: {zendesk_request}")
        if zendesk_request is None:
            zendesk_request = {}
        ...
        logger.info(f"HaloManager.update_ticket updated_ticket: {updated_ticket}")
    ...
    def add_comment(self, zendesk_request: dict = None):
        logger.info(f"HaloManager.add_comment zendesk_request: {zendesk_request}")
        if zendesk_request is None:
            zendesk_request = {}
        ticket_data = zendesk_request.get("ticket", {})
        logger.info(f"HaloManager.add_comment ticket_data: {zendesk_request}")
